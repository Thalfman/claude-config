...
- **Exact change:** Add `~/.claude/.gitignore` with the ignore list from §2; `git init && git add -A && git commit -m "chore: snapshot pre-cleanup baseline"`. Then optionally `git remote add origin <private-repo>` and `git push -u origin main`.
...
4. **Frontmatter scan (R7):** walk every `~/.claude/skills/**/SKILL.md` and `~/.claude/plugins/marketplaces/*/skills/**/SKILL.md`. For each, parse the YAML frontmatter and check:
   ...
   - Write findings to `~/.claude/_audits/2026-05-11-skill-frontmatter.md` (gitignored under `_audits/` or committed — your call)
...
> "Execute Phase 0 of `~/.claude/plans/mission-you-are-replicated-canyon-v2.md`: initialize git in `~/.claude`, write the `.gitignore` per the plan, commit the baseline, push to <remote>. Do not do anything else. Report the commit SHA, total tracked file count, and remote URL. Pause for my approval before Phase 1."
